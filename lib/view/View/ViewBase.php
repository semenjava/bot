...
use View\Helper\Form\InstanceTag\ViewHelperFormInstanceTagBase;
use View\Helper\Form\InstanceTag\ViewHelperFormInstanceTagForm;
use View\Helper\Text\ViewHelperTextCycle;
...
/**
 * Abstract base class for View to get private constructs out of
 * template scope.
 *
 * @package View
 */
abstract class ViewBase
{
     ...
     * @var string
     */
    public static $defaultFormBuilder = 'ViewHelperFormBuilder';
    ...
    public function __call($method, $args)
    {
        ...
        throw new ViewException('Helper for ' . $method . ' not found.');
    }
     ...
     *
     * To clear all paths, use View::setTemplatePath(null).
     *
     ...
     * have "Helper" in them will be prefixed with View_Helper_; other
     * names will be treated as literal class names. Examples:
     *
     * <code>
     * // Adds a new View_Helper_Tag to the view:
     * $v->addHelper('Tag');
     ...
     * </code>
     *
     * @param View_Helper|string $helper  The helper instance to add.
     *
     * @return View_Helper  Returns the helper object that was added.
     * @throws View_Exception
     */
    public function addHelper($helper)
    {
        if (is_string($helper)) {
            ...
            if (!class_exists($class)) {
                throw new ViewException('Helper class ' . $helper . ' not found');
            }
    ...
    public function assign($array)
    {
        foreach ($array as $key => $val) {
            if (isset($this->_protectedProperties[$key])) {
                throw new ViewException('Cannot overwrite internal variables in assign()');
            }
            $this->$key = $val;
        }
    }
    ...
    protected function _template($name)
    {
        ...
        if (!count($this->_templatePath)) {
            throw new ViewException('No template directory set; unable to locate ' . $name);
        }
        ...
        throw new ViewException("\"$name\" not found in template path (\"" . implode(':', $this->_templatePath) . '")');
    }
