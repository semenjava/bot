 ...
 * @subpackage Helper
 */
class ViewHelperText extends ViewHelperBase
{
    /**
     * @var array
     */
    ...
    public function pluralize($count, $singular, $plural = null)
    {
        if ($count == '1') {
            ...
            if (!$this->_inflector) {
                $this->_inflector = new Support_Inflector();
            }
    ...
    public function cycle($firstValue)
    {
        ...
        if (empty($this->_cycles[$name]) ||
            ...
            $this->_cycles[$name] = new ViewHelperTextCycle($values);
        }
    ...
    public function truncate($text, $length = 30, $truncateString = '...')
    {
        ...
        return String::length($text) > $length
            ? rtrim(String::substr($text, 0, $length - String::length($truncateString))) . $truncateString
            : $text;
    }
    ...
    public function truncateMiddle($str, $maxLength = 80, $joiner = '...')
    {
        ...
        $maxLength = $maxLength - String::length($joiner);
        if ($maxLength <= 0) {
            return $str;
        }
        $startPieceLength = (int) ceil($maxLength / 2);
        $endPieceLength = (int) floor($maxLength / 2);
        $trimmedString = rtrim(String::substr($str, 0, $startPieceLength)) . $joiner;
        if ($endPieceLength > 0) {
            $trimmedString .= ltrim(String::substr($str, (-1 * $endPieceLength)));
        }
